...
/**
 * The abstract base class for renderers of traces in a section viewer.
 ...
 */
public abstract class SeismicDatasetRenderer extends SectionViewRenderer implements ControlListener, ColorMapListener,
    IModelListener {
  ...
  /** The trace section currently displayed. */
  protected TraceSection _section;

  /** The seismic dataset to render. */
  protected Entity _seismicDataset;
   ...
   */
  protected SeismicDatasetRendererModel _model;
  ...
  /** The list of traces in the section. */
  ...
  public SeismicDatasetRenderer(final String name) {
    super(name);
    _model = new SeismicDatasetRendererModel();
    _model.addListener(this);
  }

  @Override
  public Object[] getRenderedObjects() {
    return new Object[] { _seismicDataset };
  }
  ...
  public synchronized void redraw(final boolean reload) {
    final TraceSection section = getViewer().getTraceSection();
    /*
      System.out.println("Redrawing seismic section " + section + " " + _section);
      if (section == null) {
        _section = section;
        _rebuildPixels = true;
        renderImage();
    ...
    */
    boolean readTraces = reload;
    if (section != null && _section == null) {
      readTraces = true;
    }

    if (readTraces || section != null && !section.equals(_section)) {
      _section = section;

      _model.setRecomputePercentile();
      ReadTracesTask readTracesTask = new ReadTracesTask(section, SeismicDatasetRenderer.this);
      TaskRunner.runTask(readTracesTask, "Reading traces...", TaskRunner.INTERACTIVE);
  ...
  public synchronized void setReadResult(final ReadTracesResult result) {
    final Trace[] traces = result.getTraces();
    ...
    Display.getDefault().asyncExec(new Runnable() {
      ...
      @Override
      public void run() {
        _section = result.getTraceSection();
  ...
  public boolean hasDataBeenSet() {
    return _seismicDataset != null;
  }

  /**
   * Reads the traces for the specified section from the associated seismic dataset.
   * 
   * @param monitor the progress monitor.
   * @param section the section to be rendered.
   * @return the traces for the specified section from the associated seismic dataset.
   */
  public abstract Trace[] readTraces(IProgressMonitor monitor, TraceSection section);
  ...
  /**
   * Computes the data statistics for the traces in the section.
  ...
  /**
     ...
     */
  public void updateRendererModel(final SeismicDatasetRendererModel model) {
  ...
  protected synchronized void renderImage() {
    ...
    // If the section has not yet been sec, then simply return.
    if (_section == null) {
      return;
    }
    ...
    // Compute the image bounds, based on the trace section and viewable window.
    Rectangle imageBounds = computeImageBounds(modelSpace);
    // In the case where the section is outside the current view bounds, the image bounds
    // returned have negative dimensions. If this happens, simply remove the plot image
    // and null it out. It will be reconstructed when the section moves back into view.
  ...
  private Rectangle computeImageBounds(final IModelSpace modelSpace) {
    ...
    // Determine the min and max x,y pixel coordinates of the trace section.
    double xmin = 1;
    double xmax = _section.getNumTraces();
    double ymin = _section.getStartZ();
    double ymax = _section.getEndZ();
    Point2D.Double sectionTopLeft = new Point2D.Double();
    _canvas.transformModelToPixel(modelSpace, xmin, ymin, sectionTopLeft);
    Point2D.Double sectionTopRight = new Point2D.Double();
    _canvas.transformModelToPixel(modelSpace, xmax, ymax, sectionTopRight);
    int sectionMinX = (int) Math.floor(Math.min(sectionTopLeft.getX(), sectionTopRight.getX()));
    int sectionMaxX = (int) Math.ceil(Math.max(sectionTopLeft.getX(), sectionTopRight.getX()));
    int sectionMinY = (int) Math.floor(Math.min(sectionTopLeft.getY(), sectionTopRight.getY()));
    int sectionMaxY = (int) Math.ceil(Math.max(sectionTopLeft.getY(), sectionTopRight.getY()));

    // Determine the min and max x,y pixel coordinates of the image to render.
    int imageMinX = (int) Math.max(sectionMinX, windowMinX);
    int imageMaxX = (int) Math.min(sectionMaxX, windowMaxX);
    int imageMinY = (int) Math.max(sectionMinY, windowMinY);
    int imageMaxY = (int) Math.min(sectionMaxY, windowMaxY);
  ...
  public SeismicDatasetRendererModel getSettingsModel() {
    return _model;
  }
  ...
  protected void addPopupMenuActions() {
    Dialog dialog = new SeismicDatasetRendererDialog(getShell(), _seismicDataset.toString(), this);
  ...
  protected void setNameAndImage() {
    ...
    setName(_seismicDataset);
    setImage(ModelUI.getSharedImages().getImage(_seismicDataset));
  }
  ...
  //    _canvas = _viewer.getModelSpaceCanvas();
  //    _model.setDataUnit(_seismicDataset.getDataUnit());
  //  }
  ...
  public HistogramData getHistogramData() {
    ...
    return new HistogramData(_seismicDataset.getDisplayName(), values, Float.NaN, 100, dataMin, dataMax, new RGB(255,
        0, 0));
  }

  class ReadTracesTask extends BackgroundTask {

    private final TraceSection _traceSection;

    private final SeismicDatasetRenderer _renderer;

    public ReadTracesTask(final TraceSection traceSection, final SeismicDatasetRenderer renderer) {
    ...
    public Object compute(final ILogger logger, final IProgressMonitor monitor) throws CoreException {
      ReadTracesResult result = new ReadTracesResult(_traceSection, _renderer.readTraces(monitor, _traceSection));
      _renderer.setReadResult(result);
      return result;
    }
  }
