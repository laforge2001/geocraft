...
/**
 * The model of rendering properties for a <code>PostStack3d</code> entity
 * in the histogram view.
 */
public class PostStack3dRendererModel extends Model {

  /** Enumeration of the types of data bounds to use for 3D seismic histograms. */
  public enum DataBounds {
    /** Uses the entire volume in the histogram. */
    USE_ALL_DATA("Use all data"),
    /** Uses every 10th inline and xline in the histogram. */
    USE_SPARSE_DATA("Use space data (every 10th inline and xline)"),
  ...
  public PostStack3dRendererModel() {
    ...
    _dataBounds = addEnumProperty(DATA_BOUNDS, DataBounds.class, DataBounds.USE_SPARSE_DATA);
    _aoi = addEntityProperty(AREA_OF_INTEREST, AreaOfInterest.class);
  }

  public PostStack3dRendererModel(PostStack3dRendererModel model) {
    this();
    updateFrom(model);
  }
