...
import org.geocraft.ui.waveletviewer.renderer.WaveletRenderer;
import org.geocraft.ui.waveletviewer.renderer.WaveletViewRenderer;


public class WaveletViewer extends AbstractDataViewer implements IWaveletViewer, ICursorListener {

  /** The logger. */
  private static ILogger LOGGER = ServiceProvider.getLoggingService().getLogger(WaveletViewer.class);

  /** The wavelet viewer's model of display properties */
  private WaveletViewerModel _model;

  /** The plotting structure. */
  private IPlot _plot;

  public SimpleToolBar _legendToolbar;

  private boolean _plotBoundsWanted = false;

  private double[] _plotBounds;
  ...
  protected void initializeLayeredModel(final ILayeredModel model, final Map<String, IViewLayer> folderLayers) {
    folderLayers.put(WAVELET_FOLDER, new FolderLayer(WAVELET_FOLDER, "Folder containing wavelets"));
  ...
  // Defines the bounds of the plot
  public void setPlotBounds(final double xMin, final double xMax, final double yMin, final double yMax) {

    int nBounds = 4;
    double[] plotBounds = new double[nBounds];
    plotBounds[0] = xMin;
    plotBounds[1] = xMax;
    plotBounds[2] = yMin;
    plotBounds[3] = yMax;
  ...
  /**
   * Adds objects to the wavelet viewer.
  ...
  public void addObjects(final Object[] objects, final boolean block) {
    ...
    final BackgroundTask task = new BackgroundTask() {
      ...
      /**
       * The task for adding entities to the wavelet viewer.
      ...
      public Void compute(final ILogger logger, final IProgressMonitor monitor) {
        // Begin the task.
        monitor.beginTask("Adding objects to the wavelet viewer", 1 + objects.length);
        try {
            ...
            public void run() {
              ...
              for (IConfigurationElement config : configsMulti) {
                ...
                try {
                  ...
                  rendererMulti.setData(shell, WaveletViewer.this, objects);
                } catch (Exception ex) {
              ...
              for (Object obj : objects) {
                ...
                for (IConfigurationElement config : configsSingle) {
                  ...
                  try {
                    ...
                    if (!rendererExists) {
                      newRenderer.setData(shell, WaveletViewer.this, new Object[] { object });

                      // Reset plot Bounds (if wanted)
                      if (_plotBoundsWanted) {
                        double xMin = _plotBounds[0];
                        double xMax = _plotBounds[1];
                        double yMin = _plotBounds[2];
                        double yMax = _plotBounds[3];
                        WaveletViewer.this._plot.getActiveModelSpace().setViewableBounds(xMin, xMax, yMin, yMax);
                      }

                      // Put wavelet info on the tool bar
                      Wavelet wavelet = (Wavelet) newRenderer.getRenderedObjects()[0];
                      IPlotPolyline waveletPolyline = (IPlotPolyline) newRenderer.getShapes()[0];
    ...
    if (block) {
      ...
      TaskRunner.runTask(task, "Add entities to the wavelet viewer");
    } else {
  ...
  private static IModelSpace initializeModelSpace() {
    String modelSpaceName = "Default Time-Amplitude Space";
    Unit xUnit = UnitPreferences.getInstance().getTimeUnit();
    String xLabelStr = "Time (" + xUnit.getSymbol() + ")";
    ILabel xAxisLabel = new Label(xLabelStr, Orientation.HORIZONTAL, Alignment.CENTER, true);
    IAxis xAxis = new Axis(xAxisLabel, xUnit, new AxisRange(-1000, 1000), Orientation.HORIZONTAL);
    ILabel yAxisLabel = new Label("Amplitude", Orientation.VERTICAL, Alignment.CENTER, true);
    IAxis yAxis = new Axis(yAxisLabel, Unit.SEISMIC_AMPLITUDE, new AxisRange(-1000, 1000), Orientation.VERTICAL);
  ...
  protected void setRendererData(IRenderer renderer, Shell shell, Object[] objects, boolean autoUpdate) {
    ((WaveletRenderer) renderer).setData(shell, this, objects, autoUpdate);
  }
