...
public class ExtractArea extends StandaloneAlgorithm {
    ...
    if (key.equals(_inputGrid.getKey())) {
      if (!_inputGrid.isNull()) {
        String outputName = _inputGrid.get().getMapper()
            .createOutputDisplayName(_inputGrid.get().getDisplayName(), "_extract");
  ...
  public void run(IProgressMonitor monitor, ILogger logger, IRepository repository) throws CoreException {
    ...
    float[][] outputData = extractGridData(inputGrid, aoi, monitor);
  ...
  public float[][] extractGridData(final Grid3d inputGrid, final AreaOfInterest aoi, final IProgressMonitor monitor) {
