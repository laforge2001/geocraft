...
/**
 * Decimates a grid in both the column (inline) and row (xline) directions.
 */
public class Decimate extends StandaloneAlgorithm {
  ...
  public Decimate() {
    ...
    _colDecimation = addIntegerProperty("Column Decimation", 1);
    _rowDecimation = addIntegerProperty("Row Decimation", 1);
    ...
    if (key.equals(_inputGrid.getKey())) {
      if (!_inputGrid.isNull()) {
        String outputName = _inputGrid.get().getMapper()
            .createOutputDisplayName(_inputGrid.get().getDisplayName(), "_decimate");
  ...
  public void run(IProgressMonitor monitor, ILogger logger, IRepository repository) throws CoreException {
    ...
    float[][] outputData = decimateGridData(inputGrid, rowDecimation, colDecimation, monitor);
