...
public final class Grid3dRendererModel extends Model implements IGridRendererConstants {
  ...
  private final EnumProperty<BlendingType> _blendingType;
  ...
  public Grid3dRendererModel() {
    ...
    _blendingType = addEnumProperty(BLENDING_TYPE, BlendingType.class, BlendingType.NONE);
   ...
   */
  public void setColorBarRange(final Grid3d grid) {
   ...
   */
  public void setBlendingType(final BlendingType blendingType) {
   ...
   */
  public ColorBar getColorBar(final Grid3d grid) {
    int percentile = _percentile.get();
    ColorBar colorBar = _colorBar.get();
    if (_recomputePercentile && percentile > 0) {
      float[] clipped = MathUtil.computePercentiles(grid.getValues(), grid.getNullValue(), percentile);
