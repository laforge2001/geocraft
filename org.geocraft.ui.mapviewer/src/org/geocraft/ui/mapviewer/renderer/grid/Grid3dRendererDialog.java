...
public class Grid3dRendererDialog extends PreferencesModelDialog implements ColorMapListener, IGridRendererConstants {
   ...
   */
  public Grid3dRendererDialog(final Shell shell, final String title, final Grid3dRenderer renderer, final Grid3d referenceGrid) {
    super(shell, title);
    _renderer = renderer;
  ...
  protected void buildModelForms(final IModelForm[] forms) {
    ...
    section.addComboField(BLENDING_TYPE, BlendingType.values());
    ISpecification filter = new GridGeometrySpecification(_referenceGrid.getGeometry());
  ...
  public void propertyChanged(final String triggerKey) {
    ...
    if (triggerKey != null) {
      if (triggerKey.equals(BLENDING_TYPE)) {
        BlendingType blendingType = model.getBlendingType();
        switch (blendingType) {
          case NONE:
  ...
  public IModel createModel() {
    Grid3dRendererModel settingsModel = _renderer.getSettingsModel();
    Grid3dRendererModel model = new Grid3dRendererModel(settingsModel);
