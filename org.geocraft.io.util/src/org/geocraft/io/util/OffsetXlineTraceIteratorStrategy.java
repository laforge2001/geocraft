 ...
 * volume along the offset primary direction and xline secondary direction.
 */
public class OffsetXlineTraceIteratorStrategy extends AbstractTraceIteratorStrategy {

  /** The volume over which to iterate. */
  protected PreStack3d _prestack3d;

  /**
   * Constructs a PreStack3d iterator for reading in the offset/xline direction, without an area-of-interest.
   * 
   * @param prestack3d the PreStack3d volume to read.
   */
  public OffsetXlineTraceIteratorStrategy(final PreStack3d prestack3d, final float startZ, final float endZ) {
    this(prestack3d, null, startZ, endZ);
  }

  /**
   * Constructs a PreStack3d iterator for reading in the offset/xline direction, with an area-of-interest.
   * 
   * @param prestack3d the PreStack3d volume to read.
   * @param aoi the area-of-interest.
   */
  public OffsetXlineTraceIteratorStrategy(final PreStack3d prestack3d, final AreaOfInterest aoi, final float startZ, final float endZ) {
  ...
  public Trace[] readNext() {
    ...
    if (_nextLoop >= numLoops) {
      _isDone = true;
      _message = "Done reading offsets,xlines...";
      return new Trace[0];
    }
    int offsetIndex = _nextLoop / _numXlines;
    ...
    _message = "Reading offset " + offset + ", xline " + xline + ".";
